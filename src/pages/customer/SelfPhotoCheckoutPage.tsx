...
import { ArrowLeft, Plus, Minus, Clock, Users } from 'lucide-react';
...
const SelfPhotoCheckoutPage = () => {
  ...
  const { data: pendingBookings = [], refetch: refetchPendingBookings } = useQuery({
    queryKey: ['pending-self-photo-bookings-with-transactions', userProfile?.id],
    queryFn: async () => {
      ...
      if (error) {
        console.error('Error fetching pending self photo bookings:', error);
        return [];
      }
  ...
  useEffect(() => {
    if (selectedDate && packageData?.studios?.id) {
      fetchBookedSlots();
    }
  }, [selectedDate, packageData?.studios?.id]);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-8 py-8 md:py-12">
        ...
        {pendingBookings.length > 0 && (
          <Card className="border border-orange-200 bg-orange-50 mb-8">
            <CardHeader className="p-4 md:p-6">
              <CardTitle className="font-peace-sans font-black text-orange-800 flex items-center gap-2">
                <Clock className="h-5 w-5" />
                Booking Pending
              </CardTitle>
            </CardHeader>
            <CardContent className="p-4 md:p-6 pt-0">
              <p className="text-orange-700 font-inter mb-4">
                Anda memiliki {pendingBookings.length} booking yang belum diselesaikan. Silakan selesaikan pembayaran terlebih dahulu.
              </p>
              <div className="space-y-3">
                {pendingBookings.map((booking) => (
                  <div key={booking.id} className="bg-white rounded-lg p-4 border border-orange-200">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600 font-inter mb-2">
                          {formatDateTimeWITA(booking.start_time)} - {formatDateTimeWITA(booking.end_time).split(' ')[1]} WITA
                        </p>
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          <Badge className={`text-xs font-inter ${getStatusColor(booking.status)}`}>
                            {getStatusText(booking.status)}
                          </Badge>
                          <Badge variant="outline" className="text-xs font-inter">
                            {getTypeText('self_photo')}
                          </Badge>
                        </div>
                        <p className="text-sm text-gray-600 font-inter">
                          Status Pembayaran: {getPaymentStatusText(booking)}
                        </p>
                        <p className="text-lg font-peace-sans font-bold text-primary">
                      ...
                      <Button
                        ...
                        className="bg-orange-600 text-white hover:bg-orange-700 font-peace-sans font-bold"
                      >
                        Bayar Sekarang
                      </Button>
                    </div>
        ...
        {/* Package Details */}
        <Card className="border border-gray-100 shadow-none mb-8">
          <CardHeader className="p-4 md:p-6">
            <CardTitle className="font-peace-sans font-black text-black">Detail Paket</CardTitle>
          </CardHeader>
          <CardContent className="p-4 md:p-6 pt-0">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h3 className="text-xl md:text-2xl font-peace-sans font-black text-black mb-2">
                  {packageData.title}
                </h3>
                <p className="text-gray-600 font-inter mb-4">{packageData.description}</p>
                <div className="flex items-center gap-4 text-sm text-gray-500 font-inter mb-4">
                ...
                <p className="text-2xl md:text-3xl font-peace-sans font-black text-primary">
          ...
          <Card className="border border-gray-100 shadow-none bg-gray-50 mb-8">
            ...
            <CardContent className="p-4 md:p-6 pt-0 space-y-4">
              ...
              <Button
                ...
                disabled={bookingLoading || pendingBookings.length > 0}
