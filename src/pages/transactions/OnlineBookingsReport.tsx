...
const OnlineBookingsReport = () => {
  ...
  const chartData = useMemo(() => {
    ...
    const durationPriceData = bookingsData.map(booking => {
    ...
    <div className="space-y-6 p-6">
      ...
      <Tabs defaultValue="revenue" className="space-y-4">
        ...
        <TabsContent value="revenue" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Grafik pemasukan dari booking online per hari</CardDescription>
            </CardHeader>
            <CardContent>
              <ChartContainer
                config={{
        ...
        <TabsContent value="correlation" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Korelasi Durasi vs Harga</CardTitle>
              <CardDescription>Hubungan antara durasi sesi dan total pembayaran</CardDescription>
            </CardHeader>
            <CardContent>
              <ChartContainer
                config={{
              ...
              >
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart data={chartData.durationPriceData}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="duration" name="Durasi (menit)" />
                    <YAxis dataKey="price" name="Harga" />
                    <ChartTooltip
                      content={<ChartTooltipContent />}
                      formatter={(value, name) => [
                        name === 'price' ? `Rp ${Number(value).toLocaleString('id-ID')}` : `${value} menit`,
                        name === 'price' ? 'Harga' : 'Durasi'
                      ]}
                    />
