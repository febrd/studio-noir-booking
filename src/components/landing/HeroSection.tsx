...
const HeroSection = () => {
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden px-4 sm:px-6 lg:px-8">
      ...
      <div className="hidden md:block absolute top-40 right-20 animate-pulse">
        <div className="w-12 h-12 bg-yellow-400 rotate-45 opacity-80" />
      </div>
      <div className="hidden md:block absolute bottom-32 left-20 animate-ping">
        <div className="w-8 h-8 bg-blue-500 rounded-full opacity-60" />
      </div>

      <div className="relative z-10 max-w-6xl mx-auto text-center w-full">
        <div className="flex justify-center mb-6 sm:mb-8">
          <div className="bg-white/20 backdrop-blur-lg rounded-full p-4 sm:p-6 border border-white/30">
            <Camera className="w-12 h-12 sm:w-16 sm:h-16 text-purple-600" />
          </div>
        </div>

        <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-bold bg-gradient-to-r from-purple-600 via-blue-600 to-yellow-500 bg-clip-text text-transparent mb-4 sm:mb-6 font-playfair leading-tight">
          Masuk Studio
        </h1>
        
        <p className="text-lg sm:text-2xl md:text-3xl text-gray-700 mb-6 sm:mb-8 font-light px-4">
        ...
        <p className="text-base sm:text-lg md:text-xl text-gray-600 mb-8 sm:mb-12 max-w-4xl mx-auto leading-relaxed px-4">
          Dari Limboto, kami hadir untuk bikin setiap momen terlihat seindah rasanya, dengan sentuhan kreatif dan hasil akhir yang bikin kamu bangga.
        </p>

        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-12 sm:mb-16 px-4">
          <Link to="/auth">
            <Button size="lg" className="bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white px-6 sm:px-8 py-3 sm:py-4 text-base sm:text-lg rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 w-full sm:w-auto">
